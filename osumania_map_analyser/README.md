...
osu!mania Map Analyser 是面向 [tosu](https://tosu.app) 的游戏内叠加界面（ppcounter）。它可在 osu!mania 4K、6K、7K，以及 lazer、stable 和多种 Mod 环境下实时估算难度、分析 RC/LN 键型、计算 MSD，并显示难度图表与暂停检测结果。
...
- **功能性设置**：
    ...
    - **Estimator Algorithm**: 选择用于难度估计的算法。
        ...
        - Sunny: 使用内置 `sunny-rework-js` 兼容实现直接映射段位星数，适配4/6/7K的LN与RC段位。
